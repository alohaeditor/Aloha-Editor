 ...
 */
GENTICS.Utils.Dom.prototype.removeRange = function (rangeObject) {
	if (!rangeObject) {
		// no range given
		return false;
	}
...
GENTICS.Utils.Dom.prototype.recursiveRemoveRange = function (rangeTree, rangeObject) {
	// iterate over the rangetree objects of this level
	for (var i = 0; i < rangeTree.length; ++i) {
...
GENTICS.Utils.Dom.prototype.searchWordBoundary = function (container, offset, searchleft) {
	...
	while (!boundaryFound) {
		...
		if (container.nodeType == 3) {
			...
			if (!searchleft) {
				// search right
				var wordBoundaryPos = container.data.substring(offset).search(this.nonWordRegex);
				...
				// search left
				var wordBoundaryPos = container.data.substring(0, offset).search(this.nonWordRegex);
