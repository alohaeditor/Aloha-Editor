...
define([
	 ...
	 */
	function isLastNode (node, stopNodeName) {
		if (!node.parentNode) {
			return true;
		}
	...
	Aloha.Markup = Class.extend({
		...
		preProcessKeyStrokes: function (event) {
			...
			if (event.keyCode === 38 || event.keyCode == 40) {
				if (Aloha.getSelection().getRangeCount()) {
					rangeObject = Aloha.getSelection().getRangeAt(0);
					this.processCursorUpDown(rangeObject, event.keyCode);
				}
		...
		processCursorUpDown: function (range, keyCode) {
			...
			if (keyCode === 38 && 
				...
				// cursor
				var cursorNode = tableWrapper.previousSibling;
			...
			// DOWN
				...
				// and now find its following sibling where we will put the cursor in the first position
				var cursorNode = tableWrapper.nextSibling;
				while (cursorNode.nodeType !== 3) {
