	...
	var shims = {
		...
		indexOf: function (find, i /*opt*/ ) {
			...
			if (native_method) {
				...
				if (i in obj && obj[i] === find) return i;
				return -1;
			}
		},
		...
		forEach: function (action, that /*opt*/ ) {
			...
			if (native_method) {
				...
				if (i in obj) action.call(that, obj[i], i, obj);
			}
		},
		...
		map: function (mapper, that /*opt*/ , chain /*opt */ ) {
			...
			if (native_method) {
				...
				for (var i = 0, n = obj.length; i < n; i++)
				if (i in obj) other[i] = mapper.call(that, obj[i], i, obj);
				result = other;
			}
		...
		filter: function (filterFunc, that /*opt*/ , chain /*opt */ ) {
			...
			if (native_method) {
				...
				var other = [],
					v;
				for (var i = 0, n = obj.length; i < n; i++)
				if (i in obj && filterFunc.call(that, v = obj[i], i, obj)) other.push(v);
				result = other;
			}
		...
		every: function (tester, that /*opt*/ ) {
			...
			if (native_method) {
				...
				for (var i = 0, n = obj.length; i < n; i++)
				if (i in obj && !tester.call(that, obj[i], i, obj)) return false;
				return true;
			}
		},
		...
		some: function (tester, that /*opt*/ ) {
			...
			if (native_method) {
				...
				for (var i = 0, n = obj.length; i < n; i++)
				if (i in obj && tester.call(that, obj[i], i, obj)) return true;
				return false;
			}
		},
