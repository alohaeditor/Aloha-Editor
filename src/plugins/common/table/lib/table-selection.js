define(
['aloha', 'jquery', 'table/table-plugin-utils', 'table/table-cell', 'ui/dialog', 'i18n!table/nls/i18n'],
...
function (Aloha, $, Utils, TableCell, Dialog, i18n) {
	...
	function isMergeable(grid, contour, isSelected) {
		var mergeable = true;
		if (-1 !== Utils.indexOfAnyBut(contour.top, contour.top[0]) || -1 !== Utils.indexOfAnyBut(contour.right, contour.right[0]) || -1 !== Utils.indexOfAnyBut(contour.bottom, contour.bottom[0]) || -1 !== Utils.indexOfAnyBut(contour.left, contour.left[0])) {
	...
	TableSelection.prototype.mergeCells = function () {
		...
		Utils.walkGridInsideRect(grid, selectedRect, function (cellInfo, x, y) {
			if (x - cellInfo.spannedX === selectedRect.left && y - cellInfo.spannedY === selectedRect.top) {
				return;
			}
			...
			for (var i = 0; i < contents.length; i++) {
				if ("string" !== typeof contents[i] || "" !== $.trim(contents[i])) {
		...
		$firstCell.attr({
			'rowspan': selectedRect.bottom - selectedRect.top,
