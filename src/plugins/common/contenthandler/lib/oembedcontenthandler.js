...
function (Aloha, jQuery, ContentHandlerManager) {
	...
	function getRequestUrl(provider, externalUrl) {
		...
		var url = provider.apiendpoint,
			qs = "",
			callbackparameter = provider.callbackparameter || "callback",
			i;
	...
	function initializeProviders() {
		...
		if (!isNullOrEmpty(settings.allowedProviders)) {
			for (i = 0; i < jQuery.fn.oembed.providers.length; i++) {
				if (jQuery.inArray(jQuery.fn.oembed.providers[i].name, settings.allowedProviders) >= 0) activeProviders.push(jQuery.fn.oembed.providers[i]);
			}
		...
		if (!isNullOrEmpty(settings.disallowedProviders)) {
			for (i = 0; i < activeProviders.length; i++) {
				if (jQuery.inArray(activeProviders[i].name, settings.disallowedProviders) < 0) restrictedProviders.push(activeProviders[i]);
			}
	...
	function getDefaultOEmbedProvider(defaultOEmbedProvider) {
		...
		if (defaultOEmbedProvider == "embed.ly") url = "http://api.embed.ly/v1/api/oembed?";
	...
	function isNullOrEmpty(object) {
		...
		if (jQuery.isArray(object) && object.length == 0) return true;
		return false;
	}
	...
	jQuery.fn.oembed.OEmbedProvider = function (name, type, urlschemesarray, apiendpoint, callbackparameter) {
		...
		function getUrlSchemes(urls) {
			...
			if (jQuery.isArray(urls)) return urls;
			return urls.split(";");
		}
	};
	...
	var OEmbedContentHandler = ContentHandlerManager.createHandler({
		...
		handleContent: function (content) {
			...
			content = jQuery(content).oembed(jQuery(content).text(), {
				embedMethod: "replace"
			});
