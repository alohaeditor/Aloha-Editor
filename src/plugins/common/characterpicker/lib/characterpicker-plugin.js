...
function( Aloha, jQuery, Plugin, FloatingMenu, i18n, i18nCore ) {
	...
	return Plugin.create('characterpicker', {
		...
		languages: ['en'],
		config: [ 'characterpicker' ],
		init: function() {
			var self = this;
			
			Aloha.bind('aloha-editable-activated', function ( event, data ) {
				self.applyConfig( data.editable.obj );
			});
		},
		...
		applyConfig: function ( obj ) {
			...
			that.config = that.getEditableConfig( obj );
			
			that.settings = {};
			
			/*
			if ( Aloha.settings.plugins.characterpicker && Aloha.settings.plugins.characterpicker.characters ) {
				that.settings.characters = Aloha.settings.plugins.characterpicker.characters;
			}*/

			if ( typeof that.config !== 'undefined' ) {
				that.settings.characters = that.config;
			} else if ( !that.settings.characters ) {
				that.settings.characters = characters;
			}

			if ( jQuery.isArray(that.settings.characters) ) {
				that.settings.characters = that.settings.characters.join(', ');
			}
			
			if ( that.settings.characters.length > 0 ) {
				var insertButton = new Aloha.ui.Button({
					'name': 'characterpicker',
					'iconClass': 'aloha-button-characterpicker',
					'size': 'small',
					'onclick': function(element, event) { that.characterOverlay.show(element.btnEl.dom); },
				...
				that.characterOverlay = new CharacterOverlay(that.settings.characters, that.onCharacterSelect);
			}
		},
