...
function(aQuery, i18n, i18nCore){
	...
	return {
					...
					onInit     : function () {
						 var that = this,
							 content = this.setContent(
								...
								'</fieldset></div>' + 
								'<div class="' + sb.nsClass('title-container') + '" ><fieldset><legend>' + i18n.t('field.img.title.label') + '</legend><input type="text" class="' + sb.nsClass('imageTitle') + '" /></fieldset></div>').content; 
		...
		nsSel: function () {
			...
			jQuery.each(arguments, function () { stringBuilder.push('.' + (this.pl == '' ? prefix : prefix + '-' + this.pl)); });
