define(['jquery', 'ui/component', 'ui/utils', 'jqueryui'], function (
$,
Component,
Utils) {
	'use strict';
	...
	MenuButton.makeMenuButton = function (props) {
		...
		var expand = Utils.makeButtonElement({
		...
		if (props.click) {
			action = Utils.makeButton(Utils.makeButtonElement({
				'class': 'aloha-ui-menubutton-action'
			}), props).click(props.click);

			Utils.makeButton(expand, {}, true);

			buttonset = $('<div>').buttonset().append(action).append(expand);
		} else {
			Utils.makeButton(expand, props, true).addClass('aloha-ui-menubutton-single');
		}
		...
		expand.click(function () {
			...
			if (props.siblingContainer) {
				props.siblingContainer.find('.aloha-ui-menubutton-menu').each(function () {
					if (this !== menu[0]) {
						hideMenu($(this));
					}
				});
