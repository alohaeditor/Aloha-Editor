		...
		hotKey: { 
			...
			init: function () {
				...
				Aloha.bind('aloha-editable-activated',function (e, params) {
					...
					// handle hotKeys
					var $body = jQuery('body'); // FIXME: need a universal way to search the toolbar
					if ( $body.find('.action.bold').length == 1 )
						params.editable.obj.bind( 'keydown.aloha.format', me.hotKey.formatBold, function(e) { 
							me.addMarkup( 'b' ); e.stopPropagation(); e.preventDefault(); return false; });
					else if ( $body.find('.action.strong').length == 1 )
						params.editable.obj.bind( 'keydown.aloha.format', me.hotKey.formatBold, function(e) { 
							me.addMarkup( 'strong' ); e.stopPropagation(); e.preventDefault(); return false; });
					if ( $body.find('.action.italic').length == 1 ) 
						params.editable.obj.bind( 'keydown.aloha.format', me.hotKey.formatItalic, function(e) { 
							me.addMarkup( 'i' ); e.stopPropagation(); e.preventDefault(); return false; });
					else if ( $body.find('.action.emphasis').length == 1 ) 
						params.editable.obj.bind( 'keydown.aloha.format', me.hotKey.formatItalic, function(e) { 
							me.addMarkup( 'em' ); e.stopPropagation(); e.preventDefault(); return false; });
					// ctrl-u throws the view source code window in Firefox
					//if ( $body.find('.action.underline').length == 1 )
					//	params.editable.obj.bind( 'keydown.aloha.format', me.hotKey.formatUnderline, function(e) { 
					//		me.changeMarkup( 'u' ); e.stopPropagation(); e.preventDefault(); return false; });
