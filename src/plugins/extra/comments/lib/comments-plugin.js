...
function (Aloha, Plugin, jQuery, i18n, i18nCore) {
	...
	var dom_util = GENTICS.Utils.Dom,
		clss = 'aloha-comments',
		uid = +(new Date),
		add_box = $('<div class="' + clss + '-addbox">' + '<div class="' + clss + '-content">' + '<h2>Comment:</h2>' + '<input class="' + clss + '-user" value="" />' + '<div class="' + clss + '-user-err-msg"></div>' + '<textarea></textarea>' + '<div class="' + clss + '-text-err-msg"></div>' + '<ul class="' + clss + '-colors"></ul>' + '<button class="' + clss + '-cancel ' + clss + '-btn">Cancel</button>' + '<button class="' + clss + '-submit ' + clss + '-btn">Comment</button>' + '<div class="' + clss + '-clear"></div>' + '</div>' + '<div class="' + clss + '-arrow">' + '<div class="' + clss + '-arrow-inner"></div>' + '</div>' + '</div>'),
		view_box = $('<div class="' + clss + '-viewbox">' + '<div class="' + clss + '-content">' + '<h2>Comment:</h2>' + '<textarea></textarea>' + '<ul class="' + clss + '-colors"></ul>' + '<button class="' + clss + '-submit">Submit</button>' + '<div class="' + clss + '-clear"></div>' + '</div>' + '<div class="' + clss + '-arrow">' + '<div class="' + clss + '-arrow-inner"></div>' + '</div>' + '</div>'),
		current_comment,
		comments_hash = {};
	...
	return Plugin.create('format', {
		...
		init: function () {
			...
			$.each(this.colors, function (k, v) {
				ul.append(
				$('<li title="' + k + '" style="background-color:' + v + '"></li>').click(function () {
					that.setColor(k);
				}));
			});
		...
		createBar: function () {
			var that = this,
				bar = this.bar = $('<div class="' + clss + '-bar">' + '<div class="' + clss + '-bar-shadow"></div>' + '<div class="' + clss + '-bar-toggle">' + '<div class="' + clss + '-bar-toggle-img"></div>' + '</div>' + '<div class="' + clss + '-bar-inner">' + '<h2>' + 'Comments:' + '</h2>' + '<ul></ul>' + '<div class="' + clss + '-bar-bottom">' + '</div>' + '</div>' + '</div>').click(function () {
			...
			this.bar.find('.' + clss + '-bar-toggle').click(function () {
				if (that.isBarOpen) {
		...
		revealComments: function () {
			if (this.isRevealing) {
				$('.' + clss + '-active').removeClass(clss + '-active').css('background-color', '');
			} else {
				$.each(this.comments, function (id, comment) {
					comment.elements.addClass(clss + '-active').css('background-color', comment.color);
				});
			}
		...
		openModal: function (comment) {
			...
			add_box.show().css('height', 'auto').find('input').val(this.user);
		...
		highlight: function (comment) {
			comment.elements.css('background-color', comment.color).addClass(clss + '-active')
			...
			.siblings(':not(' + '.' + clss + '-addbox,' + '.' + clss + '-ancestor,' + '.' + clss + '-bar,' + '.aloha-floatingmenu' + ')').addClass(clss + '-grayed');
		...
		highlightElement: function (element) {
			...
			element.contents().each(function () {
				var el = (this.nodeType == 3) ? $(this).wrap('<span class="' + clss + '-cleanme">').parent() : $(this);
		...
		removeHighlight: function () {
			$('.' + clss + '-grayed').removeClass(clss + '-grayed').css('opacity', '');

			$('.' + clss + '-active').removeClass(clss + '-active').css('background-color', '');

			$('.' + clss + '-ancestor').removeClass(clss + '-ancestor')
		...
		hover: function (comment, onenter) {
			...
			if (!el.hasClass(clss + '-active')) {
				if (onenter) {
					el.addClass(clss + '-hover').css('background-color', comment.color);
				} else {
					el.removeClass(clss + '-hover').css('background-color', '');
				}
			}
		},
		...
		setBarScrolling: function () {
			...
			this.bar.find('.' + clss + '-bar-inner').css({
				height: $(window).height(),
				'overflow-y': (bottom.top > this.bar.height()) ? 'scroll' : 'auto'
			});

			this.bar.find('.' + clss + '-bar-shadow').css('height', this.bar.height());
		},
		...
		printThread: function (el, comment) {
			var that = this,
				li = $('<li data-aloha-comment="' + comment.id + '">' + '<div class="' + clss + '-bar-comment">' + '<img src="' + that.getGravatar(comment.email, 40) + '" alt="" />' + '<div style="float:left;">' + '<span>' + comment.email + ' says:</span>' + '<div>' + comment.comment + '</div>' + '</div>' + '<div class="' + clss + '-clear"></div>' + '</div>' + '</li>');
		...
		insertReplyTools: function (li) {
			var that = this,
				reply = li.addClass(clss + '-bar-comment-active').find('.aloha-comments-bar-comment>.' + clss + '-bar-reply');

			if (reply.length == 0) {
				reply = $('<div class="' + clss + '-bar-reply">' + '<input value="' + this.user + '" />' + '<textarea>Replying...</textarea>' + '<button>Reply</button>' + '</div>');
				...
				reply.css('height', 0).animate({
					height: h
				}, 250, 'easeOutExpo');

				reply.find('input, textarea').css('width', reply.width() - 12);
		...
		submitReply: function (reply_tool) {
			...
			if (typeof mom == 'object') {
				var id = clss + '-' + (++uid),
					...
					text = reply_tool.find('textarea').val().trim().replace(/[\r\n]/g, '<br />'),
					index = mom.kids.push({
						id: id,
		...
		insertComment: function (comment) {
			comments_hash[comment.id] = this.comments[comment.id] = comment;
		},
		...
		preloadImages: function () {
			$.each(['hr.png', 'textbox.png'], function () {
