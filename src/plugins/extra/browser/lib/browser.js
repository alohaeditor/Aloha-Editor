...
'aloha/jquery', 'util/class', 'i18n!browser/nls/i18n', 'aloha/console',
// this will load the correct language pack needed for the browser
'browser/locale', 'css!browser/css/browsercombined.css',
//'browser/vendor/jquery.ui',
'browser/vendor/ui-layout', 'browser/vendor/jquery.jqGrid', 'browser/vendor/jquery.jstree'
	...
	var Browser = Class.extend({
		...
		preloadImages: function () {
			...
			jQuery.each(['arrow-000-medium.png', 'arrow-180.png', 'arrow-315-medium.png', 'arrow-stop-180.png', 'arrow-stop.png', 'arrow.png', 'control-stop-square-small.png', 'folder-horizontal-open.png', 'folder-open.png', 'magnifier-left.png', 'page.png', 'picture.png', 'sort-alphabet-descending.png', 'sort-alphabet.png'], function () {
		...
		callback: function (fn, cb) {
			...
			if (typeof cb !== 'function') {
				Console.warn('Unable to add a callback to "' + fn + '" because ' + 'the callback object that was given is of type "' + (typeof cb) + '". ' + 'The callback object needs to be of type "function".');
		...
		createTree: function (container) {
			...
			var header = jQuery(renderTemplate('<div class="{tree-header} {grab-handle}">\
					Repository Browser\
				</div>'));

			container.append(header, tree);

			tree.height(this.grid.height() - header.outerHeight(true)).bind('loaded.jstree', function (event, data) {
		...
		createGrid: function (container) {
			var grid = jQuery(renderTemplate('<div class="{grid} {shadow} {rounded-top}"> \
		...
		createList: function (container) {
			...
			var list = jQuery(renderTemplate('<table id="' + listUID + '"\
				class="{list}"></table>'));
			...
			this.grid.height() - (
			container.find('.ui-jqgrid-titlebar').height() + container.find('.ui-jqgrid-hdiv').height() + container.find('.ui-jqgrid-pager').height()));
			...
			container.find('.ui-pg-button').unbind().find('>span.ui-icon').each(function () {
				...
				that._pagingBtns[dir] = jQuery(this).parent().addClass('ui-state-disabled').click(function () {
		...
		createTitlebar: function (container) {
			...
			var btns = jQuery(renderTemplate('<div class="{btns}">							 \
			...
			searchField.val(prefilledValue).addClass("aloha-browser-search-field-empty").focus(function () {
				if (jQuery(this).val() == prefilledValue) {
					jQuery(this).val("").removeClass("aloha-browser-search-field-empty");
				}
			}).blur(function () {
				if (jQuery(this).val() == "") {
					jQuery(this).val(prefilledValue).addClass("aloha-browser-search-field-empty");
				}
			});
		...
		sortList: function (colModel, el) {
			...
			jQuery(el).find('span.s-ico').show().find('.ui-icon-' + colModel.sortorder).removeClass('ui-state-disabled');

			this.setSortOrder(colModel.name, colModel.sortorder).fetchItems(this._currentFolder, this.processItems);
		},
		...
		fetchItems: function (folder, callback) {
			...
			(typeof this._searchQuery === 'string') ? i18n.t('Searching for') + ' "' + this._searchQuery + '" ' + i18n.t('in') + ' ' + folder.name : i18n.t('Browsing') + ': ' + folder.name);
		...
		processItems: function (data, metainfo) {
			...
			i18n.t('Viewing') + ' ' + (from) + ' - ' + (to) + ' ' + i18n.t('of') + ' ' + (jQuery.isNumeric(this._pagingCount) ? this._pagingCount : i18n.t('numerous')));
		...
		createOverlay: function () {
			...
			if (jQuery('.aloha-browser-modal-overlay').length == 0) {
				jQuery('body').append(renderTemplate('<div class="{modal-overlay}" style="top: -99999px; z-index: 99999;"></div>'));
			}
			...
			var element = jQuery(renderTemplate('<div class="{modal-window}"  style="top: -99999px; z-index: 99999;"></div>'));
		...
		show: function () {
			...
			if (this.isFloating) {
				...
				jQuery('.aloha-browser-modal-overlay').css({
					top: 0,
					left: 0
				}).add(el).stop().show();
				...
				this.grid.css({
					...
					if (jQuery.browser.msie) {
						jQuery(this).add(el).css('filter', 'progid:DXImageTransform.Microsoft.gradient(enabled = false)');
					}
				});
			} else {
				el.stop().show().css({
