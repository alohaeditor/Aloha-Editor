 ...
 */
define(['aloha', 'aloha/plugin', 'jquery', 'ui/ui', 'ui/button', 'i18n!toc/nls/i18n', 'i18n!aloha/nls/i18n', 'aloha/console'], function (
Aloha,
Plugin,
jQuery,
Ui,
			...
			return {
				...
				cleanupIds: function (ctx) {
					...
					this.headings(this.$containers).each(function () {
						var id = jQuery(this).attr('id');
						if ((id && -1 != jQuery.inArray(id, ids)) || (ctx && (jQuery.contains(ctx, this) || ctx === this))) {
					...
					this.headings(ctx).each(function () {
						...
						var parent = detect(potentialParents, function (parent) {
							...
							detect(parentSection, function (sectionElem) {
								return $heading.get(0) === sectionElem || jQuery.contains(sectionElem, $heading.get(0));
							});
						});
						parent.push(node);
