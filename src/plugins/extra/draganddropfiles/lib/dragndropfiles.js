...
function( $, Plugin, DropFilesRepository ) {
	...
	return Plugin.create( 'draganddropfiles' , {
		...
		init: function() {
			...
			if ( draganddropfiles.settings === undefined ) {
				draganddropfiles.settings = draganddropfiles.config;
			} else {
				draganddropfiles.settings = jQuery.extend( true, draganddropfiles.config, draganddropfiles.settings );
			}
			...
			Aloha.bind( 'aloha-allfiles-upload-prepared', function( event, data ) {
				...
				if ( draganddropfiles.dropInEditable ) {
					...
					while( --len >= 0 ) {
						draganddropfiles.uploader
							.startFileUpload( draganddropfiles.filesObjs[len].id, draganddropfiles.config.upload.config );
					}
				}
			});
		},
