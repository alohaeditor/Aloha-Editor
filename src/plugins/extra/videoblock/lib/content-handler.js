...
define([
	...
	var _urlRegex = /(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/i;
	...
	var handler = Manager.createHandler({
		...
		extractVideoId: function (url) {
			var videoid = url.match(_urlRegex);
			if(videoid === null) {
			   return false;
			}
			return videoid[1];
		...
		handleContent: function (content) {
			while (content.match(_urlRegex)) {
				content = content.replace(_urlRegex, '<div class="aloha-block-VideoBlock aloha" data-aloha-block-type="VideoBlock" data-video-id="$1"></div>');
			}
			return content;
		}
	});
