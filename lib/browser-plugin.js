...
define([
	...
	var BrowserPlugin = Plugin.extend({
		...
		createList: function (container) {
			...
			var colNames = [''];
			var colModel = [{ // This is a hidden utility column to help us with auto sorting
					name	 : 'id',
					sorttype : 'int',
					firstsortorder : 'asc',
					hidden	 : true
		...
		processItems: function (data) {
			...
			this.grid.find('.ui-paging-info').html(
				'Viewing ' +		  (!data.length ? 0 : this._pagingOffset + 1)
						   + ' - '  + (!data.length ? 0 : this._pagingOffset + data.length)
