...
define(
['aloha/plugin', 'aloha/floatingmenu', 'i18n!list/nls/i18n', 'i18n!aloha/nls/i18n'],
function(Plugin, FloatingMenu, i18n, i18nCore) {
	...
	function deleteWorkaroundHandler(event) {
		var rangeObj = window.rangy.getSelection().getRangeAt(0);
		...
		var ranges = window.rangy.getSelection().getAllRanges();
		...
		if (actionPerformed) {
			window.rangy.getSelection().setRanges(ranges);
		}
