...
GENTICS.Aloha.Repositories.Uploader.addFileUpload = function(file) {
	...
	id = 'GENTICS_idx_file' + this.objects.length;
			...
			,listeners:{
				...
				,load:function(event){
					...
					if( result.success ){
						...
						GENTICS.Aloha.EventRegistry.trigger(
			        			new GENTICS.Aloha.Event('UploadSuccess', GENTICS.Aloha,{id: that.id ,file: that.file,result:result}));
