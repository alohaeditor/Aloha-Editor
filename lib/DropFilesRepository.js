...
GENTICS.Aloha.Repositories.Uploader.UploadFolder = function (data) {
	...
	this.colModel = new Ext.grid.ColumnModel([
		...
		,{header:'Size', width:60, renderer:this.fileSizeRenderer, columnTemplate:
			new Ext.XTemplate('<div class="x-grid3-cell-inner x-grid3-col-1" unselectable="on">{value}</div>')}
		
		,{header:'&nbsp;',dataIndex:'attributes', renderer:this.statusIconRenderer, columnTemplate:
			new Ext.XTemplate('<div class="x-grid3-cell-inner x-grid3-col-1" unselectable="on">{value}</div>')}
		
		,{header:'Status',dataIndex:'attributes', renderer:this.fileSizeRenderer, columnTemplate:
			new Ext.XTemplate('<div class="x-grid3-cell-inner x-grid3-col-1" unselectable="on">{value}</div>')}
		
		,{header:'Progress', renderer:this.fileProgressRenderer, columnTemplate: new Ext.XTemplate(
...
Ext.extend(GENTICS.Aloha.Repositories.Uploader.UploadFolder, GENTICS.Aloha.Repository.Folder, {
	statusIconRenderer: function(value, metaData, record, rowIndex, colIndex, store) {
		...
		if (d.length > 0 ) {
			if (!d[0].ulStatus) {d[0].ulStatus = 'Pending';}
			switch(d[0].ulStatus){
			default:
				return value;
			case 'Pending':
	...
	fileSizeRenderer: function(value, metaData, record, rowIndex, colIndex, store) {
		...
		if (d.length > 0 ) {
			return this.columnTemplate.apply({
				value: Ext.util.Format.fileSize(d[0].file.size)
			});
		}
	},
