...
[
	...
	'css!image/vendor/ui/ui-lightness/jquery-ui-1.8.10.custom.css',
	'image/vendor/ui/jquery-ui-1.8.10.custom.min',
	'image/vendor/jcrop/jquery.jcrop.min',
	'css!image/vendor/jcrop/jquery.jcrop.css'
],

function AlohaImagePlugin (Plugin, FloatingMenu) {
			...
			Aloha.bind('aloha-selection-changed', function(event, rangeObject, originalEvent) {
				...
				if(Aloha.activeEditable !== null) {
					...
					if ( foundMarkup ) {
						// img found
						me.insertImgButton.hide();
						FloatingMenu.setScope(me.getUID('image'));
						...
						FloatingMenu.userActivatedTab = me.i18n('floatingmenu.tab.img');
					} else {
						if(me.settings.config.img.ui.meta) {
		...
		initCropButtons: function() {
			...
			jQuery('body').append(
				'<div id="aloha-CropNResize-btns">\
					<button class="cnr-crop-apply" title="' + this.i18n('Accept') + '">&#10004;</button>\
					<button class="cnr-crop-cancel" title="' + this.i18n('Cancel') + '">&#10006;</button>\
				</div>'
			);
