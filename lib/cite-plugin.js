...
function CiteClosure ($, Plugin, FloatingMenu, Format, i18nCore) {
	...
	return Plugin.create('cite', {
				...
				sidebar.addPanel({
					...
					onActivate: function (effective) {
						var id = effective.attr('data-cite-id'),
							idx = that.getIndexOfCitation(id),
		...
		addCiteToReferences: function (uid) {
			...
			wrapper.append(
				'<sup id="{ref}" contenteditable="false"><a href="#{note}">[{index}]</a></sup>'.supplant({
					ref	  : ref,
					note  : note,
					index : (this.getIndexOfCitation(uid) + 1) + ''
				})
			);
		...
		addCiteDetails: function (uid, link, note) {
			...
			// Add citation information into reference list item
