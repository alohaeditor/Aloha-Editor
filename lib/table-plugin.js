...
//		}
  ...
  Table.prototype.attachLastCellEvents = function () {
	...
	Table.prototype.deleteColumns = function() {
		...
		// if a selection was made, delete the selected cells
//		if (TableSelection.selectedCells.length > 0) {
//			for (var i = 0; i < TableSelection.selectedCells[0].length; i++) {
//				colIDs.push(TableSelection.selectedCells[0][i].cellIndex);
//			}
//		}
...
//			}
	...
	Table.prototype.addRows = function(position, highlightNewRows) {
		...
		if (TableSelection.selectedCells.length > 0) {
			if (numCols) {
				rowsToInsert = TableSelection.selectedCells.length / numCols;
			}
   ...
   */
	Table.prototype.selectColumns = function ( columns ) {
		...
		// ====== BEGIN UI specific code - should be handled by UI =======
...
//          jQuery(rowCells[1]).attr('scope') == 'col'
  ...
  Table.Cell.prototype.startCellSelection = function(){
    if(!TableSelection.cellSelectionMode){
      ...
      TableSelection.baseCellPosition = [row_no, column_no];
    }
  };
  ...
  Table.Cell.prototype.selectCellRange = function(){
    ...
    var row_increment = (TableSelection.baseCellPosition[0] > row_no) ? [row_no, TableSelection.baseCellPosition[0]] : [TableSelection.baseCellPosition[0], row_no];
    var col_increment = (TableSelection.baseCellPosition[1] > column_no) ? [column_no, TableSelection.baseCellPosition[1]] : [TableSelection.baseCellPosition[1], column_no];

    var current_selection_range = expand_range(row_increment, col_increment); 
    var previous_selection_range = TableSelection.lastSelectionRange || [];
    ...
    for(var k=0; k < current_selection_range.length; k++){
      ...
      var index_of_selected_cell = TableSelection.selectionIndex(cell_to_select);

      if(index_of_selected_cell < 0){
        TableSelection.selectedCells.push(cell_to_select);
      }
    ...
    for(var l=0; l < previous_selection_range.length; l++){
      ...
      var cell_index_to_remove = TableSelection.selectionIndex(cell_to_unselect);

      TableSelection.selectedCells.splice(cell_index_to_remove, 1);
      jQuery(cell_to_unselect).removeClass(selectClass);
    }

    TableSelection.lastSelectionRange = cached_selection_range;
  };
   ...
   * also possible value is 'cell', which defines custom cell selections
	...
	TableSelection.prototype.selectColumns = function ( columnsToSelect ) {
        ...
        for ( var i = 0; i < columnsToSelect.length; i++ ) {
        	for ( var j = 0; j < this.selectedColumnIdxs.length; j++ ) {
        		if ( columnsToSelect[i] == this.selectedColumnIdxs[j] ) {
        			return;
        		}
        	}
        }
...
//		this.selectedCells.push( selectedCells );
