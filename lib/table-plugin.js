		...
		if (deleteTable) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				for (var j = 0; j < colIDs.length; j++) {
					...
					var cellIdx = gridColumnToCellIndex(rows, i, id);
					if ( !cellIdx[0] ) {
						if (cellIdx[2] ) {
						...
						var nextCellIdx = gridColumnToCellIndex(rows, i, id + 1);
						if ( !nextCellIdx[0] && nextCellIdx[2] ) {
							changeColspan.push({
								row: i,
								cell: cellIdx[1]
							});
						} else {
							cols2delete.push( cells[ cellIdx[1] ] );
						}
					}
				}
			}
			for ( var i = 0; i < changeColspan.length; i++ ) {
				var cell = jQuery (rows[ changeColspan[i].row ].cells[ changeColspan[i].cell ] );
