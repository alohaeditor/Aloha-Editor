...
//    emptyCell.html('\u00a0');
	...
	Table.prototype.columnSelectionMouseDown = function (jqEvent) {
		...
		if (jqEvent.metaKey) {
			var arrayIndex = jQuery.inArray(this.clickedColumnId, columnsToSelect);
			if (arrayIndex >= 0) {
				columnsToSelect.splice(arrayIndex, 1);
			}else{
				columnsToSelect.push(this.clickedColumnId);
			}
		}else if (jqEvent.shiftKey) {
			...
			columnsToSelect = [this.clickedColumnId];
		}
  ...
  Table.prototype.attachLastCellEvents = function () {
				...
				for (var j = 0; j < colIDs.length; j++) {
					...
					if ( cellInfo = gridColumnToCellIndex(rows, i, id) ) {
						...
						if ( !cellInfo[0] ) {
							...
							var nextcellInfo = gridColumnToCellIndex(rows, i, id + 1);
							...
							if ( nextcellInfo && !nextcellInfo[0] && nextcellInfo[2] ) {
								...
								cellToDelete.push( cells[ cellInfo[1] ] );
							}
						}
					}
				}
	...
	Table.prototype.addColumns = function( position ) {
		var 
			...
			currentColIdx,
			columnsToSelect = [],
			selectedColumnsIdx = TableHelper.selectedColumnsIdx;
		...
		if ( typeof TablePlugin.activeTable != 'undefined' ) {
			...
			if ( position == 'left' ) {
				selectedColumnsIdx.sort( function (a,b) { return a - b; } );
			} else {
				selectedColumnsIdx.sort( function (a,b) { return b - a; } );
			}
			
			for (var i = 0; i < rows.length; i++) {
							
				for (var j = 0; j < selectedColumnsIdx.length; j++) {
					
					// prepare the cell to be inserted
					cell = emptyCell.clone();
					cell.html('\u00a0');

					currentColIdx = selectedColumnsIdx[j];
   ...
   */
	Table.prototype.selectColumns = function ( columns ) {
		...
		if ( columnsToSelect &&  columnsToSelect.length > 0 &&
    		 rows && rows[0] &&	rows[0].cells && rows[0].cells[columnsToSelect[0]] ) {
			if ( rows[1] && rows[1].cells && rows[1].cells[columnsToSelect[0]] ) {
        		TablePlugin.columnHeader.setPressed(
        			// take 1 column to detect if the header button is pressd
        			rows[0].cells[columnsToSelect[0]].nodeName.toLowerCase() == 'th' &&
        				rows[1].cells[columnsToSelect[0]].nodeName.toLowerCase() == 'th'
        		);
			} else {
		...
		for (var i = 0; i < rows.length; i++) {
			...
			for (var j = 0; j < columnsToSelect.length; j++) {
				...
				if ( i == 0 ) {
					TableHelper.selectedColumnsIdx.push( columnsToSelect[j] );
				}
				
				colIndex = gridColumnToCellIndex(rows, i, columnsToSelect[j]);
				if ( colIndex[0] ) {
...
//          jQuery(rowCells[1]).attr('scope') == 'col'
   ...
   * also possible value is 'cell', which defines custom cell selections
	 ...
	 */
	TableHelper.prototype.selectedColumnsIdx = new Array();

	/**
	 * Holds all table rowIds if selectiontype is column
	 */
	TableHelper.prototype.selectedRowsIdx = new Array();
    ...
    if(this.cellSelectionMode || this.keepCellsSelected)
      return;
		...
		if (this.selectedCells.length > 0) {
			...
			this.selectedColumnsIdx = new Array();
			this.selectedRowsIdx = new Array();
