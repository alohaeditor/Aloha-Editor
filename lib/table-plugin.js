  ...
  /**
	...
	TableHelper.prototype.mergeCells = function(){
      ...
      //iterate through remaining cells
        ...
        // TODO: There's a duplication in merged content. have to check
      ...
      var rowspan_to_set = (parseInt(firstCell.attr('rowspan')) || 1) + (maxRowId - minRowId);
      var colspan_to_set = (parseInt(firstCell.attr('colspan')) || 1) + (maxColId - minColId);
      firstCell.attr({ 'rowspan': ((rowspan_to_set < 0) ? 1 : rowspan_to_set), 'colspan': ((colspan_to_set < 0) ? 1 : colspan_to_set) });
      ...
      jQuery.each(this.selectedCells, function(){
        ...
        if(colspan > 1){
          for(var i = 0; i < (colspan - 1); i++){
            cur_cell.after('<td></td>'); 
            var new_cell = new Table.Cell(cur_cell.next(), TablePlugin.activeTable);
            new_cell.activate();
          } 

          cur_cell.attr('colspan', 1);
        };

        if(rowspan > 1){
          cur_cell.attr('rowspan', 1);

          var cur_cell_x = jQuery(cur_cell).prevAll('td, th').length;
          var rows_after_current = jQuery(cur_cell).parent().nextAll('tr');

          for(var i = 0; i < (rowspan - 1); i++){
            var cell_below = jQuery(jQuery(rows_after_current[i]).children('td')[cur_cell_x]);
            cell_below.before('<td></td>');
            //TODO: correctly add new cell to the current table object
            var new_cell = new Table.Cell(cell_below.prev(), TablePlugin.activeTable);
            new_cell.activate();
          } 
        }
