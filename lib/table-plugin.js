...
//    emptyCell.html('\u00a0');
   ...
   */
	Table.prototype.selectColumns = function () {
		 ...
		 *   by the rowspans and colspans of other cells in the table.
		 */
		function gridColumnToCellIndex(rows, rowIdx, colIdx) {
			var adjust = [];
			for (var ri = 0; ri <= rowIdx; ri++) {
				...
				for (var ci = 0; ci < cells.length; ci++) {
					...
					// Select cell if colIdx + offset from rowspan matches
					if (ri == rowIdx && ci + skip == colIdx) {
						return ci;
					}
					
					// colIdx is inside a spanned cell. Don't select
					if (ri == rowIdx && ci + skip + colspan > colIdx) {
						return -1;
					}
