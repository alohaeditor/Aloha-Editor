...
function(jQuery, Plugin, PluginManager, FloatingMenu, i18n, i18nCore) {
  ...
  */
  ...
  TablePlugin.initColumnBtns = function () {
    ...
    //Add merge/split cells buttons to the contexts of cell, row and column
    FloatingMenu.addButton(
      this.getUID('cell'),
  ...
  Table.prototype.selectRows = function () {
	...
	******************************/
    ...
    wrapper.bind('mouseup',   function(jqEvent) { that.endCellSelection(); })
    ...
    this.obj.bind('mouseup', function(jqEvent){
      // end cell selection 
  ...
  /**
   * Gives the column number for a cell, taking colspans into consideration.
   */
  Table.Cell.prototype.columnNo = function(){
  ...
  /**
   * Gives the row number for a cell
   */
  Table.Cell.prototype.rowNo = function(){
    var table = this.tableObj;
  ...
  Table.Cell.prototype.startCellSelection = function(){
    if(!TableHelper.cellSelectionMode){
      ...
      // push the starting cell into selected cells, but don't highlight it yet. 
      //TableHelper.selectedCells.push(this.obj); 

      // set the start cell position
      var row_no = this.rowNo();
      var column_no = this.columnNo();
  ...
  Table.Cell.prototype.selectCellRange = function(){
    ...
    //gets a cell by its co-ordinates 
    var get_selection_cell = function(coord){
      var selection_row = table.obj.find(rowSelector)[coord[0]];

      //take colspans into consideration
      ...
      for(var i = 0; i < cur_columns.length; i++){
        ...
        virtual_col_no += (parseInt(cur_column_obj.attr('colspan')) + i || 1);
    ...
    var expand_range = function(r, c){
      ...
      for(var i = r[0]; i <= r[1]; i++){
        for(var j = c[0]; j <= c[1]; j++){
          var cur_cell = jQuery(get_selection_cell([i, j]));
      ...
      console.log("Row Range before adding: " + range_array.toString());
      ...
      console.log("Row Range after adding: " + range_array.toString());
    ...
    for(var k=0; k < current_selection_range.length; k++){
      var cell_to_select = get_selection_cell(current_selection_range[k]);
    ...
    for(var l=0; l < previous_selection_range.length; l++){
      var cell_to_unselect = get_selection_cell(previous_selection_range[l]);

      var cell_index_to_remove = jQuery.inArray(TableHelper.selectedCells, cell_to_unselect);
  ...
  */
  ...
  /**
	...
	TableHelper.prototype.mergeCells = function(){
      ...
      var minRowId = minColId = 1000000;
      var maxRowId = maxColId = -1;
      ...
      this.selectedCells.sort(function(a, b){
        ...
        if(aRowId < bRowId){
          minRowId = (aRowId < minRowId) ? aRowId : minRowId;
          maxRowId = (bRowId > maxRowId) ? bRowId : maxRowId;
          return -1; 
        }
        else if(aRowId > bRowId){
          minRowId = (bRowId < minRowId) ? bRowId : minRowId;
          maxRowId = (aRowId > maxRowId) ? aRowId : maxRowId;
          return 1; 
        }
        //row id is equal
        else {
          //sort by column id
            if(aColId < bColId){
            minColId = (aColId < minColId) ? aColId : minColId;
            maxColId = (bColId > maxColId) ? bColId : maxColId;
            return -1; 
          }
          if(aColId > bColId){
            minColId = (bColId < minColId) ? bColId : minColId;
            maxColId = (aColId > maxColId) ? aColId : maxColId;
            return 1; 
          }
        }
      });
      ...
      //iterate through remaining cells
        ...
        firstCell.find(":first-child").append("<span>&nbsp</span>" + curCell.find(":first-child").html());

        // remove the cell
        curCell.remove();
			}
      
      //set row and col span
      firstCell.attr({ 'rowspan': (maxRowId - minRowId) + 1, 'colspan': (maxColId - minColId) + 1 });
