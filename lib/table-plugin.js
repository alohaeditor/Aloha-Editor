  ...
  */
  ...
  Table.Cell.prototype.startCellSelection = function(){
    if(!TableHelper.cellSelectionMode){
      ...
      var column_no = this.obj.prevAll('td').length;
  ...
  Table.Cell.prototype.selectCellRange = function(){
    ...
    //select the first cell if it's not already selected
    if(TableHelper.selectedCells.length == 1){
      jQuery(TableHelper.selectedCells[0]).addClass(selectClass);
    }
    ...
    var previous_selection_range = TableHelper.lastSelectionRange ? expand_range(TableHelper.lastSelectionRange[0], TableHelper.lastSelectionRange[1]) : [];
    ...
    for(var k=0; k < current_selection_range.length; k++){
      var selection_row = table.obj.find('tr')[current_selection_range[k][0]];
      var selection_cell = jQuery(jQuery(selection_row).find('td')[current_selection_range[k][1]]);

      TableHelper.selectedCells.push(selection_cell);
      selection_cell.addClass(selectClass);
    }

    //unselect cells
    for(var l=0; l < previous_selection_range.length; l++){
      var selection_row = table.obj.find('tr')[previous_selection_range[l][0]];
      var selection_cell = jQuery(jQuery(selection_row).find('td')[previous_selection_range[l][1]]);

      var cell_index_to_remove = jQuery.inArray(TableHelper.selectedCells, selection_cell);
      TableHelper.selectedCells.splice(cell_index_to_remove, 1);
      selection_cell.removeClass(selectClass);
    }

    TableHelper.lastSelectionRange = [row_increment, col_increment];
