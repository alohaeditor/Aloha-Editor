			...
			for (var j = 0; j < this.columnsToSelect.length; j++) {
				...
				if ( colIndex >= 0 ) {
					...
					if( rowCells[colIndex] ){
						toSelect.push( rowCells[colIndex] );
						selectedCellsInCol.push( rowCells[colIndex] );
					}
				}
  ...
  */
  ...
  /**
	...
	TableHelper.prototype.splitCells = function(){
		...
		var cells_to_split = (this.selectedCells.length > 0 ? this.selectedCells : jQuery([Table.Cell.lastActiveCell.obj[0]]));
		...
		if (cells_to_split.length > 0) {
			...
			// 	var $rows = $cell.parent().nextAll();
			// 	for (var j = 0; j < rowspan; j++) {
			// 		$rows

			// 		//in the first row there is already the cell being split
			// 		var numNew  = (j == 0) ? colspan - 1 : colspan;
	...
	/**
	 * Adjusts the given colIdx by the rowspans and colspans of cells in a table.
	 * @param rows the rows of the table
	 * @param rowIdx the index of the row in "rows" that contains the
	 *   cell for which the index will be returned.
	 * @param colIdx the grid column index of the cell for which the
	...
	function gridColumnToCellIndex(rows, rowIdx, colIdx) {
		var cellIndex;
		walkCells(rows, function(ri, ci, gridColumn, containsCell){
			if (ri == rowIdx && gridColumn == colIdx) {
				//retun -1 for spanned cells
				cellIndex = containsCell ? ci : -1;
				return false;
			}
	    });
		return cellIndex;
		...
		for (var ri = 0; ri <= rows.length; ri++) {
			...
			for (var ci = 0; ci < cells.length; ci++) {
				...
				while (adjust[ci + skip]) {
					if (false === callback(ri, ci, ci + skip, false)) {
						return;
					}
					adjust[ci + skip] -= 1;
					skip += 1;
				...
				for (var j = 0; j < colspan; j++) {
					...
					if (false === callback(ri, ci, ci + skip + j, containsCell)) {
						return;
					}
				}
