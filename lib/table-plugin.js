  ...
  */
  ...
  Table.Cell.prototype.startCellSelection = function(){
    if(!TableHelper.cellSelectionMode){
      ...
      var row_no = this.obj.closest('tr').prevAll('tr').length;
      var column_no = this.obj.prevAll('td,th').length;
  ...
  Table.Cell.prototype.selectCellRange = function(){
    ...
    var get_selection_cell = function(coord){
      var selection_row = table.obj.find('tr')[coord[0]];
      return jQuery(jQuery(selection_row).find('td,th')[coord[1]]);
    };
    ...
    var expand_range = function(r, c){
      ...
      for(var i = r[0]; i <= r[1]; i++){
        for(var j = c[0]; j <= c[1]; j++){
          //var cur_cell = get_selection_cell([i, j]);
    ...
    var current_selection_range = cache_selection_range = expand_range(row_increment, col_increment); 
    ...
    for(var k=0; k < current_selection_range.length; k++){
      ...
      cell_to_select.addClass(selectClass);
    }
    ...
    for(var l=0; l < previous_selection_range.length; l++){
      ...
      cell_to_unselect.removeClass(selectClass);
    }

    TableHelper.lastSelectionRange = cache_selection_range;
  };
