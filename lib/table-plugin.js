  ...
  TablePlugin.initColumnBtns = function () {
    ...
    this.columnHeader = new Aloha.ui.Button({
        ...
        onclick	  : function () {
    	    ...
    	    var 
    	  		selectedColumnIdxs = TableHelper.selectedColumnIdxs,
    	  		cell,
    	  		isHeader = TableHelper.isHeader();
    	  
			// table header
			if (that.activeTable) {
			    for (var j = 0; j < TableHelper.selectedCells.length; j++) {
			    	cell = TableHelper.selectedCells[j];
			        if ( isHeader ) {
			        ...
			        jQuery( TableHelper.selectedCells[j] ).bind( 'mousedown', function ( jqEvent ) {
			            var wrapper = jQuery(this).children('div').eq(0);
			            // lovely IE ;-)
			            setTimeout(function () {
			            	wrapper.trigger( 'focus' );
			    ...
			    if (that.activeTable) {
			        that.activeTable.refresh();
			        TableHelper.unselectCells();
			        TableHelper.selectColumns( selectedColumnIdxs );
			    }
			}
        }
    });
...
//		}
		...
		// delete the whole table
		if ( TableHelper.selectedColumnIdxs.length == this.numCols ) {
...
//			}
			...
			// build the array with the row-ids of th rows which should be deleted
			for (var i = 0; i < rows.length; i++) {
				for (var j = 0; j < TableHelper.selectedColumnIdxs.length; j++) {

					// returns an array [containsCell , cellIdx, homeRow] if cell exists
					if ( cellInfo = gridColumnToCellIndex(rows, i, TableHelper.selectedColumnIdxs[j]) ) {
						...
						if ( !cellInfo[0] ) {
							...
							var nextcellInfo = gridColumnToCellIndex(rows, i, TableHelper.selectedColumnIdxs[j] + 1);
   ...
   */
	Table.prototype.selectColumns = function ( columns ) {
		...
		TablePlugin.columnHeader.setPressed( TableHelper.isHeader() );
		...
		TableHelper.selectColumns( columnsToSelect );
...
//          jQuery(rowCells[1]).attr('scope') == 'col'
   ...
   * also possible value is 'cell', which defines custom cell selections
	 ...
	 */
	TableHelper.prototype.selectedColumnIdxs = new Array();

	/**
	 * Holds all table rowIds if selectiontype is column
	 */
	TableHelper.prototype.selectedRowIdxs = new Array();
	 ...
	 * @return void
	 */
	TableHelper.prototype.selectColumns = function ( columnsToSelect ) {
...
//		this.selectedCells.push( selectedCells );
	 ...
	 * @return boolean
	 */
	TableHelper.prototype.isHeader = function ( ) {
