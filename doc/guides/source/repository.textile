...
* Resource item data structure

endprologue.

h3. Concept

A repository is a service or data source, which provides resource items and is registered with the Repository Manager (@RepositoryManager@). The Repository Manger manages all available repositories and routes queries to all registered repositories. Any plugin may make requests to the Repository Manager for resource items, and specify one or more kinds of items it wishes to have returned, such as @website@ or @localanchor@ (where @localanchor@ could be a headline provided by a document repository implementation).
Usually a CMS back-end would provide data such as links to sites, images, videos, files that should be available during the
editing process.
The repository data are resource items that contain JSON data with at least the following attributes: @id@, @name@, @url@, @type@. All other attributes are optional and depend on the item's @type@. A repository may provide resource items of a freely definable @type@. We suggest @type@ to be a <a href="http://www.iana.org/assignments/media-types/index.html">mimetype</a>, e.g.: @image/png@, @text/html@, @video/H264@, @application/pdf@ …. However, this is _not required_.
...
The implementation should perform a full text search on all attributes and properties of the resource items.
...
* @inFolderId@ [ResourceItem] OPTIONAL This is a predicate function that tests whether or not a candidate object is a child-object of the folder object identified by the given @inFolderId@.
...
h3. Recommendation of repository item attributes
...
The object concept from CMIS is inconsistent and too complicated for Aloha Editor's needs. So we opted for a much simpler model, which allows one to implement CMIS, but is easier to use for developers.
...
* documentId ID identifies the rendition document (baseObjectType == document)
...
A document may have any number of renditions.
For example: a page document in a repository may be rendered in 3 different languages.
...
which the repository will serve back in response to a request for a specific rendition of a given object.
