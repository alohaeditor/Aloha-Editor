...
Add +common/contenthandler+ to the list of plugins you would like to load / enable.

WARNING: Add the content handler plugin as (one of) the last plugin -- plugins like formatless paste or block (which are providing contenthandler) needs to be loaded before!
...
h4. Blockelement Content Handler

The blockelement implements the behaviour documented in the section about "Empty Block Level Elements":functional_description.html#implemented-behaviour
