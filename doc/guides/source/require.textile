...
You do not need to define all parameter of the callback function but +require+ the module as needed. In order to not interfere with other modules we require all Aloha Editor modules in the +aloha+ context. The configuration is available in +Aloha.settings.requireConf+. The method +requireAloha()+ is a shorthand to require with the Aloha Editor configuration and additionally gives you the +Aloha+ object an argument if you pass only a callback function or as return if you pass no argument at all. 
...
function() {
	...
	var 
		...
		dependendmodule2 = requireAloha( 'dependendmodule2' ); // shorthand to the above
		
	requireAloha( function( localAloha ) {
...
( function() {
	...
	requireAloha( ['dependendmodule1'], function( dependendmodule1 ) {
