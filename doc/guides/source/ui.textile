...
Most components will be displayed inline (next to each other). When grouped
together in an array, then these components will be grouped together and
...
Setting responsiveMode to true will replace the default float & pin behavior of the toolbar so that it stays fixed
above the active editable and will resize to fit on devices with small screens.
