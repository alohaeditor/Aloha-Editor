...
h4. Reize rows and columns

When enabled from settings, table rows and columns can be resize by drag and drop. The cursor will change to indicate resizeable rows or columns. On some occasions, you may need to increase the table width before increasing the width of individual columns.

You can reset resized tables to its default form by using the `naturalfit` option available under Table toolbar.

WARNING: Resize functionality is not optimized for IE7. There might be bugs/glitches. Use it at your own risk.
