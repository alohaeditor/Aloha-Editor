...
empty +<p>+ only, if they are the _last_ elements. If you have an empty +<p>+ between two
non-empty +<p>+, it will not be rendered. If the last +<p>+ contains the aloha-end-br, it
would show an additional line (but this is only true for the _last_ +<p>+. Therefore, the
Implementation of Aloha Editor for Internet Exporer 7 is special, as described below.
...
For Internet Explorer 7, instead of +<br class="aloha-end-br"/>+ the zero-width spaces will be used.
