...
* remove any trailing <br /> element in block level elements if in Internet Explorer
...
When getting the editable's content (e.g. for saving) with the _getContents()_
method:
...
WARNING: The removal of trailing line break elements (<code><br/></code>) in
Internet Explorer at the point of initializing Editables is a work-around for
IE's nonconformance to the W3C specification.  The consequence of this
work-around ought to be noted:<br />In the common case of persisting changes to
documents edited with Aloha Editor, if a block level element contains numerous
trailing <code><br/></code>s, each time the document is opened for editing and
then saved, it will loose one of the trailing line breaks until eventually none
is left--even if no manual editing is done in the editable.  This would be
another reason not to use line break elements to space your content; there are
better means to achieve the same.
